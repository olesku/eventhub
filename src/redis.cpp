...
Redis::Redis(const string host, int port, const string password, int pool_size) {
  ...
  pool_opts.wait_timeout        = std::chrono::seconds(5);


  _redisInstance = std::make_unique<sw::redis::Redis>(conn_opts, pool_opts);
    ...
    for (auto& streamID : result) {
      for (auto& msgID : streamID.second) {
        for (auto& keyVals : msgID.second) {
          ...
          cout << streamName << " " << payloadID << " " << topicName << " " << payload << std::endl;
        }
      }
      cout << std::endl;
    }
