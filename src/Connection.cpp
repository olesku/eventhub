...
// If we have TCP_USER_TIMEOUT set it to 10 seconds.
  ...
  spdlog::trace("Client '{}' connected.", getIP());
...
Connection::~Connection() {
  spdlog::trace("Client '{}' disconnected.", getIP());
...
void Connection::read() {
  ...
  switch (getState()) {
    ...
    default:
      spdlog::debug("Connection {} has invalid state, disconnecting.", getIP());
      shutdown();
  }
}
...
ssize_t Connection::write(const string& data) {
  ...
  if (ret <= 0) {
    spdlog::trace("Client '{}' write error: {}.", getIP(), strerror(errno));
    _enableEpollOut();
  } else if ((unsigned int)ret < _write_buffer.length()) {
    spdlog::trace("Client '{}' could not write() entire buffer, wrote {} of {} bytes.", ret, _write_buffer.length());
