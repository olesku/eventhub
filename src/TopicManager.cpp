 ...
 */
std::pair<TopicPtr, SubscriptionId> TopicManager::subscribeConnection(ConnectionPtr connection, const std::string& topicFilter, const jsonrpcpp::Id subscriptionRequestId) {
  ...
  auto subscriptionId = it->second->addSubscriber(connection, subscriptionRequestId);

  return std::make_pair(it->second, subscriptionId);
}
...
void TopicManager::publish(const std::string& topicName, const std::string& data) {
  std::vector<TopicPtr> matchingTopics;
  {
    std::lock_guard<std::mutex> lock(_topic_list_lock);
    for (auto& topic : _topic_list) {
      if (isFilterMatched(topic.first, topicName)) {
        matchingTopics.push_back(topic.second);
      }
    }
  }

  for (const auto& topic : matchingTopics) {
    topic->publish(data);
  }
}
 ...
 */
void TopicManager::deleteTopic(const std::string& topicFilter, const TopicPtr& expectedTopic) {
  std::lock_guard<std::mutex> lock(_topic_list_lock);
  ...
  if (it->second == expectedTopic && expectedTopic->getSubscriberCount() == 0) {
