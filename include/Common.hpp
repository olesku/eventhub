...
// Read buffer size.
static constexpr unsigned int NET_READ_BUFFER_SIZE = 128;

// Max write buffer size.
static constexpr unsigned int NET_WRITE_BUFFER_MAX = (1024 * 1000) * 8;

// Hangup connection if data frame is larger than this.
static constexpr unsigned int MAX_DATA_FRAME_SIZE = (1024 * 1000) * 8;
...
static constexpr unsigned int WS_MAX_CHUNK_SIZE = 1 << 15;

// Hangup connection if control frame is larger than this.
static constexpr unsigned int WS_MAX_CONTROL_FRAME_SIZE = 1024;
